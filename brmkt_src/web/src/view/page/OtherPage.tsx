...
// interface OtherPageProps extends RouteComponentProps, AppRouteParams {}
...
//           .filter(auction => auction.prodType === 'OTHER')
...
import {
  FetchAuctionListing,
  FetchAuctionListingVariables,
  FetchAuctions,
  FetchAuctionsVariables,
  ItemStatus,
...
export function Auctions() {
  const location = useLocation()
  const [, auctionId] = (location.search || '').split('?auctionId=')
  return auctionId ? <AuctionListing auctionId={Number(auctionId)} /> : <AuctionList cursor={0} />
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function AuctionList({ cursor }: { cursor: number }) {
  const [auctionQuery, setAuctionQuery] = useState(' ')
  ...
  if (loading) {
    ...
    return (
      <div style={{ marginLeft: '135px' }} className="mw6">
        ...
        <Spacer $w4 />
        {loadMoreButton(data.auctions.hasMore, data.auctions.cursor)}
      </div>
    )
  }
}
