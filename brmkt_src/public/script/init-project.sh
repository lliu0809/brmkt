...
git clone https://github.com/rothfels/brmkt.git $SLUG
cd $SLUG
rm -rf .git

sed -i.bak "s/brmkt/$SLUG/g" **/*.tf
sed -i.bak "s/brmkt/$SLUG/g" **/*.ts
sed -i.bak "s/brmkt/$SLUG/g" **/*.yml
sed -i.bak "s/brmkt/$SLUG/g" **/*.sh
sed -i.bak "s/brmkt/$SLUG/g" **/*.json
rm -rf **/*.bak
